	...
	phi(adist(rng))
{
	...
	if (!IsStaticInitialized())
	{
		...
		model.Transform(dx::XMMatrixScaling(1.0f, 1.0f, 1.8f));

		AddStaticBind(std::make_unique<VertexBuffer>(gfx, model.vertices));

		auto pvs = std::make_unique<VertexShader>(gfx, L"ColorBlend_VS.cso");
		auto pvsbc = pvs->GetBytecode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(gfx, L"ColorBlend_PS.cso"));
